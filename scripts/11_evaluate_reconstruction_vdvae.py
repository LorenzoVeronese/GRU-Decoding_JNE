...
def visualize_corrcoef(ground_truth, predictions, net_name, layer):
    ...
    # Save the plot
    plt.tight_layout()
    plt.savefig(results_metrics_dir + f'subj_{sub}/paircorr_heatmap_first_reconstruction_{net_name}-{str(layer)}.png', dpi=300, bbox_inches='tight')
...
for (net_name,layer) in net_list:
    ...
    file_name = feats_dir + 'vdvae/{}_{}.npy'.format(net_name,layer)
    if use_other_model_as_first_step==1:
        file_name = feats_dir + 'ssr/{}_{}.npy'.format(net_name,layer)
...
for i in range(num_test):
    gen_image = Image.open(results_vae_dir + 'subj_{}/{}.png'.format(sub, i)).resize((425,425))
...
df_results.to_csv(results_metrics_dir + 'subj_{}/metrics_first_reconstruction-sub{}.csv'.format(sub, sub), index=False)
