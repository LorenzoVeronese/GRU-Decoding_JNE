...
vdvae_results_dir = base_path + 'results/vdvae/subj_{}/'.format(sub)
...
pred_latents = np.load(predicted_dir + predicted_filename)
ref_latent = stats



# GET HIERARCHICAL REPRESENTATION FROM FLATTENED
print()
print("##############################")
print('Obtaining hierarchical representation...')
...
idx = range(len(stims_test))
input_latent = latent_transformation(pred_latents[idx], ref_latent)



##### GET FIRST STAGE RECONSTRUCTION
print()
print("##############################")
print('First stage reconstruction...')
...
if not os.path.exists(vdvae_results_dir):
    os.makedirs(vdvae_results_dir)
for i in range(int(np.ceil(len(stims_test)/batch_size))):
    ...
    for j in range(len(sample_from_latent)):
        ...
        im.save(vdvae_results_dir +'{}.png'.format(i*batch_size+j))
