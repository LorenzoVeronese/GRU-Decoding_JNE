...
for epoch in range(num_epochs):
    ...
    for inputs, targets in train_loader:
        ...
        loss = criterion(outputs, targets.float())  # This is MSE loss
        loss.backward()
        optimizer.step()
    ...
    learning_rates.append(current_lr)  # Add the current learning rate to the list
    scheduler.step(val_mse)  # Still using MSE for scheduling
...
# Save the model
torch.save(model.state_dict(), regression_dir+'clipvision_nn.pth')
...
plt.tight_layout()
plt.savefig(results_metrics_dir+f'subj_{sub}/'.format(sub) + 'training_nn_clipvision.png')
plt.close()
...
with torch.no_grad():
    for inputs, targets in test_loader:
        ...
        # Compute the MSE and MAE losses for the current batch
        mse_loss = torch.mean((outputs - targets.float()) ** 2)
        mae_loss = torch.mean(torch.abs(outputs - targets.float()))
        
        # Accumulate the total MSE and MAE for averaging later
        test_mse += mse_loss.item() * inputs.size(0)
        test_mae += mae_loss.item() * inputs.size(0)
        
        # Store the individual losses for standard deviation calculation
...
np.save(predicted_dir + f'subj_{sub}/clipvision_pred.npy', test_predictions)
