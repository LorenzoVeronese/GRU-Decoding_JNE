...
parser.add_argument("-is_gt", "--is_gt",help="Subject Number",default=0)
args = parser.parse_args()
sub = int(args.sub)
is_gt = int(args.is_gt)
...
##### SET PATHS ACCORDING TO GT (sub==0) OR SUBJECT (sub==[1, 2, 5, 7])
print()
print("##############################")
...
if sub in allowed_subjs and is_gt == 0:
    ...
    images_dir = results_versatile_dir + 'subj_{}/'.format(sub)
else:
    feats_dir = eval_features_dir + 'subj_{}/ground_truth/'.format(sub)
    images_dir = results_gt_dir + 'subj_{}/'.format(sub)
