...
parser.add_argument("-alpha", "--alpha",help="Regularization of Ridge", default=60000)
...
print()
print("##############################")
print('Retrieving CLIP-vision embeddings...')
train_clip = np.load(extracted_dir + 'subj_{}/clipvision_train.npy'.format(sub))
test_clip = np.load(extracted_dir + 'subj_{}/clipvision_test.npy'.format(sub))
print(f'    Shape of train clip-vision: {train_clip.shape}')
...
print(f'    Shape of train betas: {train_betas.shape}')
...
for i in range(num_embed):
    reg = skl.Ridge(alpha=best_alphas[i], max_iter=50000, fit_intercept=True) #default alpha above
    reg.fit(train_betas, train_clip[:,i])
    reg_w[i] = reg.coef_
    reg_b[i] = reg.intercept_
    
    pred_test_latents = reg.predict(test_betas)
    pred_clip_nozscore[:, i] = pred_test_latents
    std_norm_test_latents = (pred_test_latents - np.mean(pred_test_latents,axis=0)) / np.std(pred_test_latents,axis=0)
    pred_clip[:,i] = std_norm_test_latents * np.std(train_clip[:,i],axis=0) + np.mean(train_clip[:,i],axis=0)
...
metrics_vector_compute.compute_metrics(flattened_test_clip, flattened_pred_clip_nozscore, np.mean(r2s), results_metrics_dir,'CLIPvision', sub, alpha)
flattened_pred_clip = pred_clip.reshape(len(test_clip), -1)
metrics_vector_compute.compute_metrics(flattened_test_clip, flattened_pred_clip, np.mean(r2s), results_metrics_dir,'CLIPvisionZscoreInverted', sub, alpha)
...
if not os.path.exists(predicted_dir + 'subj_{}/'.format(sub)):
    os.makedirs(predicted_dir + 'subj_{}/'.format(sub))
predicted_filename = 'clipvision_pred.npy'
print(f'Saving the predicted CLIP-vision embeddings ({predicted_filename})...')
np.save(predicted_dir + 'subj_{}/'.format(sub) + predicted_filename, pred_clip)


# Save regression weights
if not os.path.exists(regression_dir + 'subj_{}/'.format(sub)):
    os.makedirs(regression_dir + 'subj_{}/'.format(sub))
datadict = {
    'weight' : reg.coef_,
    'bias' : reg.intercept_,
}
regression_filename = f'clipvision_regression_weights_alpha{alpha}.pkl'
print(f'Saving the regression weights ({regression_filename})...')
with open(regression_dir + 'subj_{}/'.format(sub) + regression_filename,"wb") as f:
